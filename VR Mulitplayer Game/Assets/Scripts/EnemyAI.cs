...
public class EnemyAI : MonoBehaviourPun, ITakeDamage
{
    ...
    public void Shoot()
    {
        ...
        {
            RaycastHit hit;
            Vector3 direction = player.GetHeadPosition() - shootingPositing.position;
            if (Physics.Raycast(shootingPositing.position, direction, out hit))
            {
